...
// Implementation of a general inverse position kinematics algorithm based on Newton-Raphson iterations with joint limits,
// if a solution is not found, it will chose another random seed and try again.
//
...
#include <kdl/chainiksolverpos_nr_jl.hpp> // Newton-Raphson solver, with joint limits
...
class Kinematics {
    ...
    private:
        ...
        bool searchPositionIK(kinematics_msgs::GetPositionIK::Request &request,
                              kinematics_msgs::GetPositionIK::Response &response);
...
double Kinematics::genRandomNumber(const double &min, const double &max)
{
...
KDL::JntArray Kinematics::getRandomConfiguration()
{
    KDL::JntArray jnt_array;
    jnt_array.resize(num_joints);
    for(unsigned int i=0; i < num_joints; i++) // num_joints_
        jnt_array(i) = genRandomNumber( joint_min(i),joint_max(i) ); // joint_min_ max_
    return jnt_array;
} 
